...
team_abbreviations = pd.read_csv(
    os.path.join("data", "clean", "team_abbreviations.csv")
)
...
game_results = game_results.loc[
    (game_results["year"] > 2010) & (game_results["year"] != 2020)
]
...
game_results["home_teamID"] = (
    game_results["home_abbreviation"] + "-" + game_results["year"].astype(str)
)
game_results["away_teamID"] = (
    game_results["away_abbreviation"] + "-" + game_results["year"].astype(str)
)
...
game_results = game_results.assign(
    avg_age_diff=lambda x: x.home_average_age - x.away_average_age
)

game_results = game_results.drop(
    [
        "away_team",
        "home_team",
        "year",
...
for team in playoff_teams:
    all_matchups = all_matchups.append(
        pd.DataFrame(
            {
                "Home_TeamID": np.repeat(team, 15),
                "Away_TeamID": list(filter((team).__ne__, playoff_teams)),
            }
        )
    )
...
clf = LogisticRegressionCV(
    cv=10, solver="liblinear", max_iter=1000, penalty="l1", scoring="roc_auc"
).fit(game_results.iloc[:, 3:14], game_results["home_win"])

all_2022_matchups = all_matchups.iloc[:, 0:2].reindex(
    columns=["Home_TeamID", "Away_TeamID"]
)
all_2022_matchups["Home_win_prob"] = clf.predict_proba(all_matchups.iloc[:, 2:13])[:, 1]

all_2022_matchups.to_csv(
    os.path.join("artifacts", "all_2022_matchups.csv"), index=False
)
